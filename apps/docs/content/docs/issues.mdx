...
Multica has eight statuses. **Any status can move directly to any other** — Multica doesn't impose a workflow, and won't stop you from jumping from `backlog` straight to `done`.
...
Once an issue is assigned to an agent, the agent automatically moves the status from `todo` / `backlog` to `in_progress`, then to `done` on completion. You can also change it manually at any time.
