...
| `MULTICA_PRIVATE_LOGIN_CODE` | 空 | 私有小团队固定 6 位登录码。仅当 `ALLOWED_EMAILS` 非空时才生效 |
...
**设 `MULTICA_PRIVATE_LOGIN_CODE` 时的行为**：登录页改成“邮箱 + 固定 6 位码”模式。用户仍然要先输入邮箱，但不再依赖邮件送达。这个模式只适合人数很少、且 `ALLOWED_EMAILS` 已精确收紧的私有实例。
