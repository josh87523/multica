...
| `MULTICA_PRIVATE_LOGIN_CODE` | empty | Fixed 6-digit login code for tiny private teams. Only takes effect when `ALLOWED_EMAILS` is non-empty |
...
**Behavior when `MULTICA_PRIVATE_LOGIN_CODE` is set**: the sign-in flow switches to "email + fixed 6-digit code". Users still enter their own email first, but the verification step no longer depends on email delivery. This mode is only intended for tightly allowlisted private instances with a very small number of users.
